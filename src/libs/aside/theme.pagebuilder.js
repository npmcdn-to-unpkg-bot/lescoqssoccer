	...
	this.moduleload = function(data, container) {
		jQuery.post(data['aurl'], {
			...
			if (container.is('.container-isotope')) {
				...
				var newElems = jQuery(content).css({
						opacity: 0
					}),
	...
	this.contentslider = function() {
		...
		_moudle_listitemslider.each(function() {
			...
			if (slider_width < 561) {
				...
				jQuery(this).find('.carousel-img-wrap').css({
					'height': slider_img_height,
					'width': '100%',
					'float': 'none'
				});
				jQuery(this).find('.slider-panel').css({
					'width': '100%',
					'float': 'none',
					'height': '400px'
				});
				...
				jQuery(this).find('.carousel-indicators').css({
					'width': '100%'
				});
				jQuery(this).find('h2.slider-title').css({
					'font-size': '18px',
					'line-height': '20px'
				});
			...
			ux_pb_fullwrap_withtab.each(function() {
				...
				_this_link.each(function(i) {
					...
					jQuery(this).click(function() {
						...
						if (jQuery(this).hasClass('full-nav-actived')) {} else {
	...
	this.separator = function() {
		...
		_moudle_separator.each(function() {
			...
			if (_this.hasClass('title_on_left')) {
				_this_inn.css({
					'margin-left': _this_title_width + 'px'
				});
			} else if (_this.hasClass('title_on_right')) {
				_this_inn.css({
					'margin-right': _this_title_width + 'px'
				});
			}
		...
		_moudle_clients.each(function() {
			...
			_this.find("li:first-child img").imagesLoaded(function() {
				...
				_this.find('ul').carouFredSel({
					...
					scroll: column,
					prev: function() {
						return _this.find('.prev');
					},
					next: function() {
						return _this.find('.next');
					},
					auto: false
		...
		_moudle_postcarousel.each(function() {
			...
			theme_win.load(function() {
				_this.find('.post-carousel').carouFredSel({
					...
					pagination: function() {
						return _this.find('.post-carousel-pagination');
					},
				...
				_this.find('.post-carousel').animate({
					'opacity': 1
				}, 300);
