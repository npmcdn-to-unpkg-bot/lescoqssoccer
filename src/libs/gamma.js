...
angular.module('mean.albums').factory('Gamma',
	function() {
		...
		var Gamma = {
			...
			init: function(settings, name, callback) {
				...
				Gamma.settings = $.extend(true, {}, this.defaults, settings);
			...
			_createSingleView: function() {
				...
				if (!Gamma.singleview) {
					...
					this._initEvents('singleview');

					this._createSingleViewNavigation();
			...
			_createSingleViewNavigation: function() {
				...
				if (Gamma.itemsCount > 1) {
					...
					this._initEvents('singleviewnavigation');
					...
					if ($item.length) {
						...
						if (Gamma.svImage) {
							...
							if (Gamma.supportTransitions) {
								...
								self._setTransition(Gamma.svImage, 'all', Gamma.settings.svImageTransitionSpeedFade, Gamma.settings.svImageTransitionEasingFade);

							}

							self._applyAnimation(Gamma.svImage, {
								opacity: 0
							}, Gamma.settings.svImageTransitionSpeedFade, Gamma.supportTransitions, function() {

								$(this).remove();
								anim = false;
								self._singleviewitem($item, anim);

							});

							if (Gamma.svDescription) {

								self._applyAnimation(Gamma.svDescription, {
									opacity: 0
			...
			_saveState: function(id) {
				...
				if (!Gamma.settings.historyapi && id != undefined) {
					...
					Gamma.isSV ? this._goto(false, id) : this._goto(true, id);
			...
			_layout: function($items) {
				...
				$items.each(function() {
					...
					var $item = $(this),
						$picEl = $item.children(),
						sources = self._getImgSources($picEl),
						source = self._chooseImgSource(sources, $item.outerWidth(true)),
			...
			_resizeGrid: function() {
				...
				Gamma.items.each(function() {
					...
					var $item = $(this),
						source = this._chooseImgSource($item.data('source'), Gamma.items.outerWidth(true));
			...
			_svResizeImage: function(callback) {
				...
				var finalConfig = self._getFinalImgConfig({

						sources: Gamma.svImage.data('source'),
						imgMaxW: Gamma.svImage.data('maxwidth'),
						imgMaxH: Gamma.svImage.data('maxheight'),
						wrapper: {
							width: this.$window.width() - Gamma.svMargins.horizontal,
							height: this.$window.height() - Gamma.svMargins.vertical
						},
						image: {
				...
				self._applyAnimation(Gamma.svImage, finalStyle, Gamma.settings.svImageTransitionSpeedResize, Gamma.supportTransitions, function() {
					...
					if (currentSrc !== source.src) {
						...
						Gamma.svImage = $('<img/>').load(function() {
								...
								if (Gamma.supportTransitions) {
									...
									self._setTransition($img, 'all', Gamma.settings.svImageTransitionSpeedResize, Gamma.settings.svImageTransitionEasingResize);

								}

								self._applyAnimation($img.next(), {
									opacity: 0
			...
			_getFinalImgConfig: function(properties) {
				...
				if (finalSizePosition.checksource) {
					...
					source = this._chooseImgSource(sources, finalSizePosition.width);
			...
			_singleviewitem: function($item, anim) {
				...
				// need to know which source to load for the image.
				// also need to know the final size and position.
				var finalConfig = this._getFinalImgConfig({
				...
				if (Gamma.settings.overlayAnimated && anim && Gamma.supportTransitions) {
					...
					self._setTransition(Gamma.overlay, 'opacity');

				}

				setTimeout(function() {

					self._applyAnimation(Gamma.overlay, {
						'opacity': 1
					...
					if (!anim) {
						...
						self._loadSVItemFromGrid(data, finalSizePosition, source.src);
						...
						self._applyAnimation(Gamma.fly, styleCSS, Gamma.settings.speed, cond, function() {
			...
			_loadSVItemFromGrid: function(data, position, src) {
				...
				if (!Gamma.svDescription) {
					...
					if (Gamma.supportTransitions) {
						...
						self._setTransition(Gamma.svDescription, 'opacity', Gamma.settings.svImageTransitionSpeedFade / 2, Gamma.settings.svImageTransitionEasingFade);
				...
				Gamma.svImage = $('<img/>').load(function() {
					...
					if (Gamma.fly) {
						...
						if (Gamma.supportTransitions) {
							...
							self._setTransition(Gamma.fly, 'opacity', 1000);

						}
						setTimeout(function() {

							self._applyAnimation(Gamma.fly, {
								'opacity': 0
			...
			_closesingleview: function() {
				...
				var wst = this.$window.scrollTop();

				if (!$item.is(':inViewport')) {

					wst = $item.offset().top + ($item.outerHeight(true) - $item.height()) / 2;

					var diff = this.$document.height() - this.$window.height();
					...
					this.$window.scrollTop(wst);

				}

				var l = Gamma.svImage.position().left + this.$window.scrollLeft(),
					t = Gamma.svImage.position().top + wst;

				Gamma.svImage.appendTo(this.$body).css({
					position: 'absolute',
					zIndex: 10000,
					left: l,
					top: t
				...
				if (Gamma.supportTransitions) {
					...
					self._setTransition(Gamma.svImage);
				...
				setTimeout(function() {
					...
					self._applyAnimation(Gamma.svImage, styleCSS, Gamma.settings.speed, Gamma.supportTransitions, function() {
			...
			_visChange: function() {
				...
				if (Gamma.slideshow) {
					...
					isHidden() ? (self._stopSlideshow(true), Gamma.slideshow = true) : self._prepareSlideshow();
			...
			_prepareSlideshow: function() {
				...
				self._toggleControl(Gamma.svclose, 'off');
				self._toggleControl(Gamma.svnavprev, 'off', {
					left: -40
				});
				self._toggleControl(Gamma.svnavnext, 'off', {
					right: -40
				});

				self._svResizeImage(function() {
			...
			_preloadNext: function() {
				...
				var next = Gamma.current < Gamma.itemsCount - 1 ? Gamma.current + 1 :
					...
					$img = $item.children('img'),
					finalConfig = self._getFinalImgConfig({
			...
			_stopSlideshow: function(pause) {
				...
				if (!pause) {
					...
					Gamma.svMargins = Gamma.settings.svMarginsVH;
					self._toggleControl(Gamma.svclose, 'on');
					self._toggleControl(Gamma.svnavprev, 'on', {
						left: 20
					});
					self._toggleControl(Gamma.svnavnext, 'on', {
						right: 20
					});
					self._svResizeImage(function() {
				...
				switch (type) {
					...
					case 'window':
						...
						if (Gamma.settings.historyapi) {
							...
							this.$window.on('statechange.gamma', function() {
						...
						this.$window.on('smartresize.gamma', self._resize);
						...
						if (visProp) {
							...
							document.addEventListener(evtname, self._visChange);
					...
					case 'singleviewnavigation':
						...
						Gamma.svnavnext.on('click.gamma', function() {
							self._onnavigate('next');
						});
						Gamma.svnavprev.on('click.gamma', function() {
							self._onnavigate('prev');
						});
						...
						if (Gamma.settings.keyboard) {
							...
							this.$document.on('keydown.gamma', function(event) {
